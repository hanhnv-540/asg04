...
{
         AudioStream asFire = null;
//         AudioStream asMove = null;
	static BufferedImage	background;
	private BufferedImage	top, base, tread;
	Image	              explode	 = Toolkit.getDefaultToolkit().getImage(Option.imageDir + Option.FILE_SEPARATOR + "Explode-04-june.gif");
	private Graphics	  g;
	...
	int	                  tempXstart	= 0;
	int	                  tempYstart	= 0;
	private Point	      cursorLoc	 = new Point(0, 0);	                                // local variable for being able to get cursor location anywhere
	...
	public TankPaint(String back, String top, String base, String tread)
	{
       ...
       private void loadBeginPoints(){
            ...
            this.beginX = rd.nextInt(width);
            this.beginY = rd.nextInt(height);
        }
	/*private void loadBeginPoints()
	{
        ...
        private void playSound(){
            try{
                InputStream inputStream = getClass().getResourceAsStream("fire");
                AudioStream audioStream = new AudioStream(inputStream);
                AudioPlayer.player.start(audioStream);
            } catch(IOException ex){
               Logger.getLogger(TankPaint.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
...
//                        g.drawImage(explode, eX, eY, this);
                       ...
                       // playSound();
                        try {
                            String audioFile = "E:\\java\\tankGame\\Sounds\\fire.wav";
                            InputStream in = new FileInputStream(audioFile);
                            asFire = new AudioStream(in);
...
//            InputStream in;
//            try{
//                        String audioFile = "E:\\hoctap\\bangkep\\kyhe2013-2014\\java\\labs\\TankGame\\resource\\Sounds\\movetanks.wav";
//                        in = new FileInputStream(audioFile);
//                            asMove = new AudioStream(in);
...
//                        }
		...
		{
//                    if(asMove != null){
//                    AudioPlayer.player.stop(asMove);
//                    }
                    t.stop();
		}
		try
	...
	public void mousePressed(MouseEvent m)
	{
		...
		{
                     //JOptionPane.showMessageDialog(this, "gjhhgj");
			t.stop();
			if (explosion)
			{
				explosion = false;
...
//		 finalY = centerY;// + (randAngle2 % radius);
//		 }
		finalSelection = new Point(centerX-20, centerY-35);
